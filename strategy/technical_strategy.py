...
class TechnicalStrategy(Strategy):
    ...
    def _adx(self, lookback=14):
        ...
        try:
            ...
            plus_di = 100 * (plus_dm/atr).ewm(alpha=1/lookback,
                                              min_periods=lookback).mean()
            minus_di = 100 * (minus_dm/atr).ewm(alpha=1 /
                                                lookback, min_periods=lookback).mean()

            adx_df["ADX"] = 100 * abs((plus_di - minus_di) / (plus_di + minus_di)).ewm(
